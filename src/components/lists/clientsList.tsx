...
export function ClientsList({ clients }: clientListProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return clients
    return clients.filter((p) => p.name.toLowerCase().includes(q))
  }, [clients, search])
  ...
  return (
    <div className="bg-background px-2 py-4 rounded-lg mt-6">
      ...
      {paginated.length === 0 ? (
        <p className="py-8 text-center text-muted-foreground text-sm">Nenhum cliente encontrado.</p>
      ) : (
        paginated.map((client) => (
          <Item asChild key={client.id} className="border-0 border-t border-muted rounded-none">
            <div>
              <ItemContent>
                <ItemTitle>{client.name}</ItemTitle>
                <ItemDescription>
                  <span className="">{formatPhone(client.phone)}</span>
                </ItemDescription>
              </ItemContent>
              <ItemActions>
