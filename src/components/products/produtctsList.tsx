...
export function ProductList({ products }: ProductListProps) {
  ...
  return (
    <div className="bg-background px-2 py-4 rounded-lg mt-6">
      ...
      {paginated.length === 0 ? (
        <p className="py-8 text-center text-muted-foreground text-sm">Nenhum produto encontrado.</p>
      ) : (
        paginated.map((product) => (
          <Item asChild key={product.id} className="border-0 border-t border-muted rounded-none">
            <div>
              <ItemContent>
                ...
                <ItemDescription>
                  ...
                  <span className="text-laranja-500">{currencyFormatter.format(product.price)}</span>
                </ItemDescription>
              </ItemContent>
              <ItemActions>
